const app = {
    state: {
        ...
        config: {
            ...
            apiKey: localStorage.getItem('netflix_tmdb_key') || ''
        }
    },
            ...
            if (isDirectFile) {
                ...
                const initPlayer = () => {
                    ...
                    player = new YT.Player('yt-player-target', {
                        ...
                        width: '100%',
                        videoId: ytVideoId,
                        playerVars: { 'autoplay': 1, 'controls': 1, 'cc_load_policy': 0, 'iv_load_policy': 3, 'modestbranding': 1, 'rel': 0 },
                        events: {
                            'onReady': (event) => {
                ...
                if (iframeUrl.includes('drive.google.com')) {
                    ...
                    // aggressive HD force
                    iframeUrl += '?mime=video/mp4&vq=hd1080';
                }
        ...
        loadContent: async () => {
            ...
            try {
                const fetcher = async (path) => {
                    const res = await fetch(`${tmdbBaseUrl}${path}?api_key=${apiKey}&language=en-US`);
